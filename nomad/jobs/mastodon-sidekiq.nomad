    ...
    task "sidekiq" {
      ...
      service {
        ...
        check {
          ...
          task = "sidekiq"
          command = "/bin/sh"
          args = ["-c", "ps aux | grep '[s]idekiq\\ 6' && exit 0 || exit 1"]
          interval = "5s"
          timeout  = "5s"
        }
      }
      ...
      config {
        ...
        args = [
          "bash",
          "-c",
          "bundle exec sidekiq -c ${var.thread_count}"
        ]
      }
      ...
      env {
        ...
        DB_POOL                  = var.thread_count
        MAX_THREADS              = var.thread_count
